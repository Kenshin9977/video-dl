...
def ffmpeg_video(path: str, acodec_supported: bool, vcodec_supported: bool, fps: int) -> None:
    filename = os.path.splitext(path)[0]
    ext = os.path.splitext(path)[1]
    now = datetime.now()
    date_time = now.strftime(" - %Y-%m-%d-%H-%M-%S")
    recode_acodec = "aac" if not acodec_supported else "copy"
    recode_vcodec = best_encoder(
...
def progress_ffmpeg(cmd: List[str], action: str, filepath: str) -> None:
...
def get_progress_percent(timestamp: str, total_duration: int) -> int:
...
def best_encoder(path: str, ext: str, fps: int) -> str:
    now = datetime.now()
    date_time = now.strftime(" - %Y-%m-%d-%H-%M-%S")
