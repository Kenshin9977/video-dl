function Get-PSEncoder {
	...
	if ($nvenc_gen_1 -or $nvenc_gen_2 -or $nvenc_gen_3 -or $nvenc_gen_4 -or $nvenc_gen_5 -or $nvenc_gen_6){return "NVENC"}
	if ($qsv_gen_1 -or $qsv_gen_2 -or $qsv_gen_3 -or $qsv_gen_4 -or $qsv_gen_5){return "QSV"}
	return "x264"
}
...
function PSVideo-DL($ASIC, $Ini){
	...
	elseif ($ASIC -eq "x264") {$Encoder = "h264"}
	else {Write-Host "You have no encoder available"; return}
